...
from models.embeddings import embed_texts, build_icd_embedding_store, rerank_icd_candidates
...
if uploaded_files:
    ...
    from models.embeddings import build_icd_embedding_store, rerank_icd_candidates

    icd_store = build_icd_embedding_store(cpt_icd_mapping_df, lambda texts: rerank_icd_candidates.embed_texts(texts) if hasattr(rerank_icd_candidates, 'embed_texts') else embed_texts(texts))
    results = []
    ...
    for idx, uploaded_file in enumerate(uploaded_files, start=1):
        ...
        ranked_icds = rerank_icd_candidates(clean, icd_candidates, icd_store, top_k=5)
        ...
        comments_str = f"Missing: {', '.join(validation_result['missing_sections'])}" if validation_result['missing_sections'] else ""
        row = {
            ...
            "Service Description": ", ".join([cpt_icd_mapping_df[cpt_icd_mapping_df["CPT"] == cpt]["CPT Description"].iloc[0] for cpt in predicted_cpts if not cpt_icd_mapping_df[cpt_icd_mapping_df["CPT"] == cpt].empty]) if predicted_cpts else "",
            "Clinician Name": phi_data.get("Clinician", ""),
            "Note Status": "Pending",
            "Insurance Balance Status": "Pending Initial Submission",
    ...
    # Convert to DataFrame with required headers in order
    headers = [
        "Date",
        "Appointment Type",
        "Client Name",
